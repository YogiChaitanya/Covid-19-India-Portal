...
app.post('/login/', async (request, response) => {
  ...
  if (dbUser === undefined) {
    ...
    const isPasswordMatched = await bcrypt.comapre(password, dbUser.password)
...
app.put('/districts/:districtId/', async (request, response) => {
  ...
  const {districtDetails} = request.body
  const {districtName, stateId, cases, cured, active, deaths} = districtDetails
  const updateTheDistrictQuery = `UPDATE district SET district_name='${districtName}',state_id='${stateId}',cases='${cases}',cured='${cured}',active='${active}',deaths='${deaths}' WHERE district_id=${districtId};`
